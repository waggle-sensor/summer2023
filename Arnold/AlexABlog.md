...
**August 14 2023**
Big blog update. Testing out SR (namely the ZSSR method) to see if it can help with object detection frameworks. I've namely been using YOLOv8. I've got a set of 100 of the COCO validation images in normal and 2x resolution, but the results seem to be about the same and the metrics are similar if not slightly worse which was disapointing. This could be due to a couple reason: the information in COCO is already clear enough that making it bigger doesn't help, or maybe the SR isn't adding any new helpful information to the model. Worst case it could be contributing extra noise.

I also tried using the new rideshare stickers to see if super resolution increased the confidence of model 2. Ran into the same issue, but thought it might be because of the model resizing the images anyways. When I limited the resize it made the confidence drop a lot for both, so it may take a retraining of the model to actually see results with that metric. The results were also essentially the same for both sets.